module.exports = (hyenas) => {
    //Catalog
    hyenas.get('/nsx/material/8/8542c657fc54707d69f0863b970249b8e89553bc-1207262.jpg', (req, res) => 
        res.sendFile('./data/nsx/material/8/8542c657fc54707d69f0863b970249b8e89553bc-1207262.jpg', { root: __dirname }));
    //Themes
    hyenas.get('/nsx/material/f/f8a0ced0c728b032b094463a0ea34748b1be7fb2-1207269.jpg', (req, res) =>
        res.sendFile('./data/nsx/material/f/f8a0ced0c728b032b094463a0ea34748b1be7fb2-1207269.jpg', { root: __dirname }));
    //Add-Ons
    hyenas.get('/nsx/material/3/3d31e1d216647b227f6f1218a613c2491a38597e-1206977.jpg', (req, res) =>
        res.sendFile('./data/nsx/material/3/3d31e1d216647b227f6f1218a613c2491a38597e-1206977.jpg', { root: __dirname }));
    //Avatars
    hyenas.get('/nsx/material/7/7f1262f4dcbb7ef813420dc5bc415ce5bdfdf4e4-1207257.jpg', (req, res) =>
        res.sendFile('./data/nsx/material/7/7f1262f4dcbb7ef813420dc5bc415ce5bdfdf4e4-1207257.jpg', { root: __dirname }));
    //Apps
    hyenas.get('/nsx/material/6/653ab0e8355f5bc6c7177106aba0c0d401da53f9-1206981.jpg', (req, res) =>
        res.sendFile('./data/nsx/material/6/653ab0e8355f5bc6c7177106aba0c0d401da53f9-1206981.jpg', { root: __dirname }));
    //Movies
    hyenas.get('/nsx/material/7/7ac8ebd3de7c54b21a76593a79b8a55580c67477-1207265.jpg', (req, res) =>
        res.sendFile('./data/nsx/material/7/7ac8ebd3de7c54b21a76593a79b8a55580c67477-1207265.jpg', { root: __dirname }));
    //PlayStation Store - Controller BG
    hyenas.get('/nsx/material/b/b786496313e01ceda47b6c5d6ea60693ad038667-30594.png', (req, res) =>
        res.sendFile('./data/nsx/material/b/b786496313e01ceda47b6c5d6ea60693ad038667-30594.png', { root: __dirname }));
    //PlayStation Store - Movie BG
    hyenas.get('/nsx/material/b/b0d756b5a4f132829a8fe3dc3c320673aa453c31-30595.png', (req, res) =>
        res.sendFile('./data/nsx/material/b/b0d756b5a4f132829a8fe3dc3c320673aa453c31-30595.png', { root: __dirname }));
    //Youtube
    hyenas.get('/nsx/material/6/62c86f1cc8f6e341d0763977f802f55aa1dfb204-973553.png', (req, res) =>
        res.sendFile('./data/nsx/material/6/62c86f1cc8f6e341d0763977f802f55aa1dfb204-973553.png', { root: __dirname }));
    //Tubi
    hyenas.get('/nsx/material/3/3b9f2cd300d70e75b73f03d7417ae0e87450da36-1184213.jpg', (req, res) =>
        res.sendFile('./data/nsx/material/3/3b9f2cd300d70e75b73f03d7417ae0e87450da36-1184213.jpg', { root: __dirname }));
    //Netflix
    hyenas.get('/nsx/material/2/278bde480e78ec2b04cb2f32fee8e01998175329-1215059.jpg', (req, res) =>
        res.sendFile('./data/nsx/material/2/278bde480e78ec2b04cb2f32fee8e01998175329-1215059.jpg', { root: __dirname }));
    //Hulu
    hyenas.get('/nsx/material/a/abbd8278a5baa03c63d2a6b71c1e4b815e9fe743-1215060.jpg', (req, res) =>
        res.sendFile('./data/nsx/material/a/abbd8278a5baa03c63d2a6b71c1e4b815e9fe743-1215060.jpg', { root: __dirname }));
    //NHL
    hyenas.get('/nsx/material/4/4170731a05fd363219ce44246af696c9c5c1bbeb-1107703.jpg', (req, res) =>
        res.sendFile('./data/nsx/material/4/4170731a05fd363219ce44246af696c9c5c1bbeb-1107703.jpg', { root: __dirname }));
    //MLB.tv Premium
    hyenas.get('/nsx/material/0/00571da45bfbe8dd7ecf70056f4e5e59f747bc54-732629.jpg', (req, res) =>
        res.sendFile('./data/nsx/material/0/00571da45bfbe8dd7ecf70056f4e5e59f747bc54-732629.jpg', { root: __dirname }));

    //PS Exclusive - The Unfinished Swans
    hyenas.get('/nsx/material/0/00dcbd8e0def3d9340fde00422ff544aeac0e2c3-310043.jpg', (req, res) =>
        res.sendFile('./data/nsx/material/0/00dcbd8e0def3d9340fde00422ff544aeac0e2c3-310043.jpg', { root: __dirname }));
    //NFS:MW - EA In Stores Now > Buy Now
    hyenas.get('/nsx/material/7/7e0201329d2d866cd54539331d1cfe2c16e13912-317353.jpg', (req, res) =>
        res.sendFile('./data/nsx/material/7/7e0201329d2d866cd54539331d1cfe2c16e13912-317353.jpg', { root: __dirname }));
};